...
Contains parameters, data file paths, and a function to output parameters
"""
#to write to stdout.
import sys
...
foutname="test_ranvia_"+ datetime.now().strftime(dateFORMAT) + "_out.txt"

#path to a directory where all the models ot be used should be stored
modeldata_indir='metmodels\\ranviable\\minglc_r871\\'
#file where a list of models to be used is stored, together with some model properties, such as biomass growth rate
#should be stored in the same directory as modeldata_indir
modellist_infname = "ranvia_stats_minglc_871_04-11-2021.txt"

#path to a file that holds a map of KEGG metabolite ids to common metabolite names, from San Roman 2018
keggmap_indir='metmodels\\ranviable\\'

#for the random number generator
seed=12377

med_default=0  #default initial concentration of all medium components
#dictionary of the concentration of non-zero medium components
med_init={'C00031_e': 25.0} #glucose minimal medium
...
n_pert=3 #number of perturbation to be tried to examine stability of each community
tmax_eq=24 #maximal time allowed until a community must have reached equilibrium after a perturbation
tint_eq_pert=256  #perturbed community is examined every tint_eq_pert hours for equilibrium, used for equilibration after perturbation 
...
#minimal growth flux difference that cross-feeding must cause in the recipient
...
def parprint(outfilename):
    ...
    for outfile in outarr:
        ...
        print('\nmodel input directory', modeldata_indir, end ='', file = outfile)
        print('\nkeggmap input directory', keggmap_indir, end ='', file = outfile)


        print('\nseed=', seed, end ='', file = outfile)
        print('\nmed_default={0:.4f}'.format(med_default), end ='', file = outfile)
        print('\nmed_init=', med_init, end ='', file = outfile)
